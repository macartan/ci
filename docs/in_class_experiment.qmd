...
full_data <- 
  fabricate(N = 20,
          ...
          Y1e = rep(15, 20),   # Better to select many Y0s
          Y1f = 2*(20:1)*prompt + 5  # Better to block with more Y0s in prompt = FALSE group
          ) |>
  arrange(ID)
...
5. When all numbers are complete calculate:
  1. An estimate of the difference between the black and white numbers
...
7. Repeat steps 1 - 5  10 times and show histograms of
  1. the estimates
  2. the standard errors
  3. the  $p-$values
  4. the confidence interval for the estimate
8. Now turn over all the cards and calculate:
  1. the true difference between the numbers 
  2. the correlation between the white numbers, the black numbers 
  3. the correlation between the black numbers and the prompt (if any)
  4. using the calculations you have, estimate: 
      1. the power of your design
      2. the coverage of the confidence intervals
