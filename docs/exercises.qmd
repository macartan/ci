...
# Familiarity with `DeclareDesign`
...
1. What do each of these two steps do?  
2. Draw data from this design. What are the dimensions of the data set? Is this what you would expect? Can you describe what each of the columns in this dataset are?
3. Calculate the population  average treatment effect for this dataset using `Y_Z_1` and `Y_Z_0` columns. Is this what you would expect from the potential outcomes function? 
4. Diagnose this design. Why so many NAs?
...
1. What do each of the new steps do?   What is the difference between the two inquiries? Explain how they are different.
2. Draw data from this design. What are the dimensions of the data set? Is this what you would expect?
3. Diagnose the design using `diagnose_design`. Use a large number of simulations, e.g. setting `sims = 2000`. Why is there a difference in the RMSE for the two estimands?  Why is the power the same? Why is there a difference in coverage?

```{r, eval = show_answer, include = show_answer}
diagnosis_0 <- diagnose_design(design, sims = 2000)

diagnosis_0 |> reshape_diagnosis() |> kable()
...
1. Add an inquiry declaration, which should be the average treatment effect, or average difference between the treated and control potential outcomes.
...
2. Add a data strategy declaration, including an assignment step that creates a variable `Z` using simple random assignment with probability 0.5, and a measurement step in which you reveal the observed outcomes `Y` using the `reveal_outcomes` function.
...
3. Add an answer strategy declaration, which uses the `difference_in_means` function (pass it to the `.method` argument of `declare_estimator`) and calculates the difference in mean observed outcomes between the treatment and control groups. Link the estimator to the inquiry. 
...
4. Draw data from the design, and confirm that the variables you expect are present, including unit characteristics, two potential outcomes, the treatment variable, and the observed outcome. 
...
5. Run the design once using `run_design` to confirm that the estimand and estimate columns are present. What do these columns refer to?
...
6. Diagnose the design with 1000 simulations and calculate its statistical power. What is the power?
...
7. Calculate the minimum detectable effect size of the design, which means the smallest expected effect size for which the statistical power of the design reaches the common threshold of 80%. Use the `redesign` function to redesign over `expected_effect_size`'s between 0.1 and 1 by steps of 0.1 and then `diagnose` the resulting list of designs. Find the smallest expected effect size where the diagnosis reports the power is at least 0.8.

```{r, include = show_answer, eval = show_answer}
# Answer:
designs <- 
  redesign(design, expected_effect_size = seq(0.1, 1, 0.1))

diagnoses <- diagnose_designs(designs, sims = 1000)
...
8. Bonus: What is the smallest sample size would you need to obtain 80% power for an expected effect size of 0.3? You can estimate power in sample size steps of 100 and report the smallest size that exceeds 80% power.
...
bonus_designs <- redesign(bonus_design, N = seq(100, 500, 100))

bonus_diagnosis <- diagnose_designs(bonus_designs, sims = 1000)
...
1. X1 causes X2 and Y, but X1 does not cause Y through X2
2. X1 causes Y via X2 only (with no direct effect when X2 is fixed at 0)
3. X1 causes Y via X2 *and* directly
4. X1 causes X2, X2 causes Y, but (overall), X1 does not cause Y
...
1. do this by permuting Z (given X) and using a coefficient on Z from the regression Y ~ Z + X as a test statistic
2. do this by permuting Z (given X) and using a coefficient on X:Z from the regression Y ~ Z + X + X:Z  as a test statistic
...
Replicate Table 1 in [@lin2012agnostic](https://www.stat.berkeley.edu/~winston/agnostic.pdf);
...
2. Use `grid <- expand_grid(m0 = ..., m1 = ..., s0 = ..., s1 = ... )` to generate a grid of possible values for the four parameters (make sure `s0` and `s1` are positive..
...
1. $Z$ is assigned according to: $\Pr(Z=1) := \pi = 1/2 + X1/4 + \alpha X2/4$
2. Potential outcomes are given by $Y = Z + \beta X1 + X2 + \epsilon$  
...
1. a *prediction* $\hat{\pi}$ from regressing $Z$ on $X1$ (e.g. using a probit)  
2. a *prediction* $\hat{Y_z}$ from regressing $Y$ on $X2$ in each treatment group (and predicting to the other group)

Compare your estimates from:

1. IPW  (using  $\hat{\pi}$)
2. Calculating   the average of $\hat{Y_1} - \hat{Y_0}$
3. Employing the doubly robust formula from the [slides](https://macartan.github.io/ci/6_topics_1.html#/doubly-robust-estimation-1)  to get an estimate of treatment effects (don't worry about estimating standard errors) 
4. Naive model regressing Y on Z.
...
# X2 used for prediction model
y1_pred <- function(data) 
  data |>
  mutate(y0 = lm(Y~X2, subset = Z==0, data = data) |> 
  predict(newdata = data),
  y1 = lm(Y~X2, subset = Z==1, data = data) |> 
  predict(newdata = data),
  diff = y1 - y0)
...
design <- 
  declare_model(
    ...
    # outcome model correct when beta = 0
    Y = Z + beta*X1 + X2 + rnorm(N),
    p_pred = glm(Z~X1, family = binomial(link = "probit"))$fitted.values,
    ipw = 1/(Z*p_pred + (1-Z)*(1-p_pred))) +
  declare_inquiry(ate = 1) +
  declare_measurement(handler = y1_pred)  +
  declare_estimator(Y ~ Z,  label = "naive") +
  declare_estimator(Y ~ Z,  weights = ipw, label = "ipw") +
# declare_estimator(diff ~ 1, label = "prediction") +
  declare_estimator(Y ~ Z,  covariates = ~ X2, .method = lm_lin, label = "lin") +
  declare_estimator(handler = label_estimator(dr), label = "dr")
  
diagnosis <- 
  design |> redesign(alpha = c(0, .5, 1), beta = c(0, .5, 1)) |>
  diagnose_design(sims = 200)

diagnosis |> tidy() |> filter(diagnosand == "mean_estimate") |> 
  ggplot(aes(estimator, estimate)) + geom_point()  + theme_bw() +
  facet_grid(alpha~ beta, labeller = label_both) + geom_hline(yintercept = 1, color = "red") +
