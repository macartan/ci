...
Imagine a population with 100 northerners (X=0) and 100 southerners (X=0). 
...
df <-
  expand_grid(X = 0:1, Z = 0:1) |>
  mutate(Y = X + Z -  2 * X * Z) |>
  uncount(25)

ps <- function(df, sims = 5000) {
  coefs_1 <- lm(Y ~ Z, data  = df) |> coef() |> nth(2)
  coefs_2 <- lm(Y ~ Z * X, data  = df) |> coef() |> nth(4)
  
  T1 <-
    replicate(sims, lm(Y ~ Z, data  = df |> group_by(X) |> mutate(Z = sample(Z))) |>
                coef() |> nth(2))
  T2 <-
    replicate(sims,
              lm(Y ~ X * Z, data  = df |> group_by(X) |> mutate(Z = sample(Z))) |>
                coef() |> nth(4))
  
  c(p1 = mean(abs(T1) >= abs(coefs_1)), p2 = mean(abs(T2) >= abs(coefs_2)))
}
