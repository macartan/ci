...
**Hint** You can use the `fabricatr::correlate` function to generate correlated data. Also, to redesign over a range of values, assign a value to `rho` before you declare the design, and then you do  `design |> redesign(rho = c(-1, 0, 1))`, or use whatever  range you like.
...
**Hint**:  The direction of collider bias is related to the ways that $K$ and $Y$ interact to produce $X$. Also: by "conditioning on $K=1$" we mean: does the analysis using only cases for which $K=1$.
