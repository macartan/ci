...
[People often wonder: did randomization work? \* Common practice is to implement a set of $t$-tests to see if there is balance \* This makes no sense.]{style="font-size: 80%;"}

[If you doubt whether it was **implemented** properly do an $F$ test \* If you worry about **variance** specify controls in advance as a function of relation with outcomes (more on this later) \* If you worry about **conditional bias** then look at substantive differences between groups, not $t$--tests]{style="font-size: 80%;"}
...
[-   **Principle:** (unless you are worried about spillovers) generally make clusters as small as possible]{style="font-size: 80%;"}

[-   **Principle:** Surprisingly, variability in cluster size makes analysis harder. (See analysis section)]{style="font-size: 80%;"}

[-   **Be clear** about whether you believe effects are operating at the cluster level or at the individual level. This matters for power calculations.]{style="font-size: 80%;"}

[-   **Be clear** about whether spillover effects operate only within clusters or also across them. If within only you might be able to interpret treatment as the effect of being in a treated cluster...]{style="font-size: 80%;"}

### Cluster Randomization: Block by cluster size

[Surprisingly, if clusters are of different sizes the difference in means estimator is *not* unbiased, even if all units are assigned to treatment with the same probability.]{style="font-size: 80%;"}
...
The solution is to block by cluster size. For more see: [http://gking.harvard.edu/files/cluster.pdf](http://gking.harvard.edu/files/cluster.pdf)

## Blocked assignments and other restricted randomizations {#blocked}
...
-   But here be dragons
    ...
    -   General injunction [http://www.ncbi.nlm.nih.gov/pubmed/15580598](Senn 2004) \`\`as ye randomize so shall ye analyze'')
