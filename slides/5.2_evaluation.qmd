...
```{r evalsetup, message = FALSE, echo = FALSE, include = FALSE}
...
```{r dice, echo = FALSE}
include_graphics(here::here("slides", "assets/dice.jpeg"))
```
...
design |>
  ...
  write_rds(here::here("slides", "saved/simple_diagnosis_mult.rds"))

read_rds(here::here("slides", "saved/simple_diagnosis_mult.rds")) |>
    tidy() |> 
    filter(diagnosand == "power") |>
    ggplot(aes(N, estimate, color = factor(b))) +
...
design |> redesign(N = c(10, 100, 1000, 10000), rho = c(.1, .5, .9)) |>
  ...
  write_rds(here::here("slides", "saved/tolag.rds"))

read_rds(here::here("slides", "saved/tolag.rds")) |>
...
* if you difference: *the lag has to be sufficiently informative to pay its way* (the $\rho = .5$ equivalent between level and change follows from @gerber2012field equation 4.6)
...
if(run)
  diagnose_design(bad_design) |>
  write_rds(here::here("slides", "saved/bad_design_diagnosis.rds"))

read_rds(here::here("slides", "saved/bad_design_diagnosis.rds")) |>
    reshape_diagnosis() |>
    select("Mean Estimate", "Bias", "SD Estimate", "RMSE", "Power", "Coverage" ) |>
    kable()
...
if(run)
  diagnose_design(another_bad_design) |>
  write_rds(here::here("slides", "saved/bad_design_diagnosis_2.rds"))

read_rds(here::here("slides", "saved/bad_design_diagnosis_2.rds")) |>
    reshape_diagnosis() |>
    select("Mean Estimate", "Bias", "SD Estimate", "RMSE", "Power", "Coverage" ) |>
    kable()
...
write_rds(here::here("slides", "saved/clustered.rds"))

read_rds(here::here("slides", "saved/clustered.rds")) |>
  reshape_diagnosis() |>
    select("Mean Estimate", "Bias", "SD Estimate", "RMSE", "Power", "Coverage" ) |>
  kable()
...
write_rds(here::here("slides", "saved/clustered_2.rds"))

read_rds(here::here("slides", "saved/clustered_2.rds")) |>
  reshape_diagnosis() |>
    select("Mean Estimate", "Bias", "SD Estimate", "RMSE", "Power", "Coverage" ) |>
  kable()
...
simulate_designs(design_mc, sims = 5000) |>
  write_rds(here::here("slides", "saved/multiple_comparisons.rds"))

read_rds(here::here("slides", "saved/multiple_comparisons.rds"))  |>
  filter(term != "(Intercept)") |>
  group_by(sim_ID) |>
  mutate(p_bonferroni = p.adjust(p = p.value, method = "bonferroni"),
         p_holm = p.adjust(p = p.value, method = "holm"),
...
if(run)
  ...
  redesign(b = 0) |>
  diagnose_design(sims = 1000) |>
  write_rds(here::here("slides", "saved/design_mc_3.rds"))

read_rds(here::here("slides", "saved/design_mc_3.rds")) |> reshape_diagnosis() |>
    select("Mean Estimate", "Bias", "SD Estimate", "RMSE", "Power", "Coverage" ) |>
  kable()
