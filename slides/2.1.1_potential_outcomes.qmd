...
We will apply these ideas of potential outcomes to examine the problem of endogeneous subgroups, or "post treatment conditioning"
...
```{r, echo = FALSE}
make_model("X -> C <- U -> Y" ) |> plot(x_coord = c(3, 1, 3, 2), y_coord = c(2, 3, 1, 2))
```

* The conditioning set $C$ is downstream from $X$. It is also informative about $Y$, here via $U$. It *may or may not* also be a cause of $Y$. Here:
  * $X$ is "unconditionally" independent of $Y$ (and in particular, it does not cause $Y$)
  * But it is *not* independent of $Y$ *conditional on* $C$

If you condition on $C$ -- e.g. by creating subgroups or "controlling" for $C$ in other ways--- you are "conditioning on a collider."
