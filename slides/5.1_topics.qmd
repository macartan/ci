...
* Particular solution: focus on the effect of treatment *at the time of first treatment*
...
```
This is an obvious DAG but the main point is to be clear that the *Value* is the quantity of interest and the value is not affect by the treatment, *Z*.
...
**In other words**: sensitivities notwithstanding, *they are happy for the research to make correct inferences*
...
read_rds("saved/rdd_diagnosis.rds") |> reshape_diagnosis() |> 
  select(Estimator, 'Mean Estimate', 'Bias', 'SD Estimate', 'Coverage')
```
...
Another somewhat obvious approach is to see how the effect of $X$ on $Y$ in a regression is reduced when you control for $M$. If the effect of $X$ on $Y$ passes through $M$ then surely there should be no effect of $X$ on $Y$ after you control for $M$. 
...
Lets imagine that sequential ignorability does not hold. What are our posteriors on mediation quantities when in fact all effects are mediated?
...
truth <- make_model("X -> M ->Y") |> 
  set_parameters(c(.5, .5, .25, 0, .5, .25, .25, 0, .5, .25))
...
Ns |> 
  lapply(function(n)
    model |> update_model(data = truth |> make_data(n = n))) |>
  query_model(queries = queries)
...
if(run)
  ...
  lapply(function(n)
    model |> update_model(data = truth |> make_data(n = n))) |>
  query_model(queries = queries, using = "posteriors") |>
  write_rds("saved/cq-mediation.rds")

read_rds("saved/cq-mediation.rds") |> kable()
  
  
```

Punchline: you can learn something from the structure of the data