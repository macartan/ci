...
What's a cause? What's an explanation?

## Outline

1. Potential outcomes
2. Endogenous subgroups
3. DAGs and causal models 
...
* highlights a deep connection between **random assignment** and **random sampling**: when we do random assignment *we are in fact randomly sampling from different possible worlds*.
* provides a **positive argument** for causal inference from randomization, rather than simply saying with randomization "everything else is controlled for"
...
-   Say though that treated cases are likely to drop out of the sample if things go badly (eg they get a negative score or die)
...
* Endogeneous Nodes = $\{X, Y\}$, both with range $\{0,1\}$
* Exogeneous Nodes = $\{\theta^X, \theta^Y\}$, with ranges $\{\theta^X_0, \theta^X_1\}$ and $\{\theta^Y_{00}\theta^Y_{01}, \theta^Y_{10}, \theta^Y_{11}\}$
...
* So we want to learn about the distributions of the exogeneous nodes
