...
7. An illustration of power
6. A deeper dive into declaration functionality
...
read_rds("saved/redesigns.rds") |>
  select(N, m, diagnosand, estimate, std.error, conf.low, conf.high) |> slice(c(1:2, 8:9, 29:30)) |> kable(digits = 2)
```
...
designs <- design_multi |> redesign(prob_each = list(c(.2, .5, .3), c(0, .5, .5)))
...
design <- 
  ...
  declare_assignment(Z = simple_ra(N), Y = reveal_outcomes(Y ~ Z)) + 
  declare_inquiry(ate = mean(Y_Z_1 - Y_Z_0)) + 
  declare_estimator(Y ~ Z, inquiry = "ate", .method = lm_robust, label = "estimator 1")
...
design <-
  ...
  declare_estimator(Y ~ X1 + X2 + X3, term = c("X2", "X3"), inquiry = c("ate_2", "ate_3"))
...
tidy_margins <- function(x) broom::tidy(margins::margins(x, data = x$data), conf.int = TRUE)
