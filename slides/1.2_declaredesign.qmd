...
```{r midamida, child = here::here("slides", "topics/MIDA.qmd")}
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("slides", "assets", "cheat_sheet.png")) 
```
...
if(run)
  simplest_design |>
  diagnose_design() |>
  write_rds(here::here("slides", "saved/simplest_design.rds"))
```
...
diagnosis <- read_rds(here::here("slides", "saved/simplest_design.rds")) 
...
if(run)
  ...
  write_rds(here::here("slides", "saved/group_rep.rds"))

grouped_diagnosis <-   read_rds(here::here("slides", "saved/group_rep.rds")) 
...
if(run){
set.seed(1)
diagnose_design(sampling_design, sims = c(5000, 1, 1, 1)) |> write_rds(here::here("slides", "saved/multistage1.rds")) 
diagnose_design(sampling_design, sims = c(1, 5000, 1, 1)) |> write_rds(here::here("slides", "saved/multistage2.rds"))
}

list(diagnosis_1 = read_rds(here::here("slides", "saved/multistage1.rds")) |> reshape_diagnosis(),
     diagnosis_2 = read_rds(here::here("slides", "saved/multistage2.rds")) |> reshape_diagnosis()) |>
...
if(run)
  simplest_design_2 |>
  diagnose_design() |>
  write_rds(here::here("slides", "saved/simplest_design_2.rds"))
```

```{r, echo = FALSE}

read_rds(here::here("slides", "saved/simplest_design_2.rds")) |>
  reshape_diagnosis() |>
  select(-Inquiry, -Estimator, -Outcome, -Term) |>
  kable() |> 
  kable_styling(font_size = 20)
...
```{r, echo = FALSE}
if(run)
  redesign(design, N = c(100, 200, 300), m = c(0, .1, .2)) |> diagnose_design() |> tidy() |> write_rds(here::here("slides", "saved/redesigns.rds"))

read_rds(here::here("slides", "saved/redesigns.rds")) |>
...
A parameter has to be called correctly. You get a warning if you misname.
...
redesign(design, N = 50) %>%
  compare_diagnoses(design)  %>%
  write_rds(here::here("slides", "saved/compare_diagnoses.rds"))

  read_rds(here::here("slides", "saved/compare_diagnoses.rds"))$compared_diagnoses_df |>
...
if(run)
  ...
  write_rds(here::here("slides", "saved/simple_simulations_1"))

sims_1 <- 
  read_rds(here::here("slides", "saved/simple_simulations_1"))
...
if(run)
  redesign(design, b = 0) |> 
  simulate_design(sims = 10000) |>
  write_rds(here::here("slides", "saved/simple_simulations_2"))

sims_2 <- 
  read_rds(here::here("slides", "saved/simple_simulations_2"))
...
if(run)
  ...
  write_rds(here::here("slides", "saved/simple_diagnosis_1"))

read_rds(here::here("slides", "saved/simple_diagnosis_1")) |>
  reshape_diagnosis() |>
  select("Mean Estimate", "Bias", "SD Estimate", "RMSE", "Power", "Coverage" ) |>
  kable() |> 
  kable_styling(font_size = 20)
...
design |>
  redesign(b = c(0, 0.25, 0.5, 1)) |>
  diagnose_design(sims = 10000) |>
  write_rds(here::here("slides", "saved/simple_diagnosis_2"))

  read_rds(here::here("slides", "saved/simple_diagnosis_2")) |>
    reshape_diagnosis() |>
...
```{r, echo = FALSE}
if(run)
  diagnose_design(design) |> write_rds(here::here("slides", "saved/probit.rds"))

read_rds(here::here("slides", "saved/probit.rds")) |> 
  reshape_diagnosis() |>
  kable() |> 
  kable_styling(font_size = 20)
```
...
```{r, echo = FALSE}
if(run)
  diagnose_design(design) |> write_rds(here::here("slides", "saved/probit_2.rds"))

read_rds(here::here("slides", "saved/probit_2.rds")) |> reshape_diagnosis() |> kable() |> 
  kable_styling(font_size = 20)
```
