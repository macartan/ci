...
```{r, echo = FALSE}
if(run)
  design |> diagnose_design(sims = 2000) |>write_rds(here::here("slides", "saved/fixer.rds"))
diagnosis <- read_rds(here::here("slides", "saved/fixer.rds"))
```
...
```{r, echo = FALSE}
if(run)
  design_2 |> diagnose_design(sims = 1500) |>write_rds(here::here("slides", "saved/fixer2.rds"))
diagnosis <- read_rds(here::here("slides", "saved/fixer2.rds"))
```
...
-   In other words, it comes from the variance in the treatment outcomes and variance in the control outcomes since you are sampling these
...
### Neyman standard errors {.smaller}
...
* **Key result**: the variance in the treatment effect relates directly to variance in each treatment group.

$$
\begin{split}
V(\widehat{ATE})  =
\underbrace{
\frac{n}{n-1}\left[\frac{V_1}{n_T} + \frac{V_0}{n_C}\right]
}_{\begin{array}{c}
\text{This part is estimated} \\
\text{from observed variances} \\
\text{in each treatment group}
\end{array}}
-
\underbrace{
\frac{1}{n-1}\left[V_1 + V_0 - 2C_{01}\right]
}_{\begin{array}{c}
\text{This part cannot} \\
\text{be estimated but is} \\
\text{known to be positive}
\end{array}}
\end{split}
$$

Next slides build up intuition for this
...
* $Var(x) = \frac{1}n\sum_i(x_i - \overline{x})^2$. 

From the definition of variance, over $n$ repeated 'runs' of a design:
...
Here the $\hat\tau$s are the treatment effect estimates we might get under different random assignments and $\overline{\hat\tau} = \tau$ is the truth (if the estimator is unbiased).

### Variance and standard errors {.smaller}

For intuition imagine:

* we have just two units $A$, $B$
* potential outcomes $A_1$, $A_0$, $B_1$, $B_0$.

**Note**:  When there are two units with outcomes $x_1, x_2$, the variance simplifies like this:

$$Var(x) = \frac{1}2\left(x_1 - \frac{x_1 + x_2}{2}\right)^2 + \frac{1}2\left(x_2 - \frac{x_1 + x_2}{2}\right)^2 = \left(\frac{x_1 - x_2}{2}\right)^2$$

### Var(ATE)  {.smaller}

In the two unit case the two possible treatment estimates, depending on what gets put into treatment are: 
...
$$Var(\hat{\tau}) =  \left(\frac{A_1 - B_1}{2}\right)^2 + \left(\frac{A_0 - B_0}{2}\right)^2+ 2\frac{(A_1 - B_1)(A_0-B_0)}{4}$$
The first 2 terms are the variances of $Y(1)$ and  $Y(0)$. The last term is a bit pesky; it corresponds to twice the *covariance* of $Y(1)$ and $Y(0)$. 

### Why does covariance play such a role?  {.smaller}
...
* $Y_A(0) = 1$, $Y_A(1) = 2$
* $Y_B(0) = 2$, $Y_B(1) = 3$
* You will estimate either 0 or 2, so variance in estimates = 1

World 2: 

* $Y_A(0) = 1$, $Y_A(1) = 3$
* $Y_B(0) = 2$, $Y_B(1) = 2$
* You will estimate either 1 or 1, so variance in estimates = 0

Note: ATE the same; also marginal distributions of potential outcomes the same.

### And sure enough {.smaller}

World 1:

$$
\underbrace{Var(\hat{\tau})}_{1}
 =
\underbrace{\left(\frac{3 - 2}{2}\right)^2}_{\frac{1}{4}}
+
\underbrace{\left(\frac{1 - 2}{2}\right)^2}_{\frac{1}{4}}
+
\underbrace{2\frac{(2 - 3)(1 - 2)}{4}}_{\frac{1}{2}}
$$
*variance augmented by positive covariance*

World 2:

$$
\underbrace{Var(\hat{\tau})}_{0}
 =
\underbrace{\left(\frac{3 - 2}{2}\right)^2}_{\frac{1}{4}}
+
\underbrace{\left(\frac{1 - 2}{2}\right)^2}_{\frac{1}{4}}
+
\underbrace{2\frac{(3 - 2)(1 - 2)}{4}}_{-\frac{1}{2}}
$$

*variance eliminated by negative covariance*
...
-   With at least two units per arm we  can  use the sample estimates $s^2(\{Y_i\}_{i \in C})$ and $s^2(\{Y_i\}_{i \in T})$ for the first part.
...
design <-
  declare_model(N = 100, 
              Y_Z_0 = rnorm(N),
              Y_Z_0 = Y_Z_0/sd(Y_Z_0),
              Y_Z_1 = correlate(rnorm, given = Y_Z_0, rho = rho),
              Y_Z_1 = Y_Z_1/sd(Y_Z_1)) +
  declare_measurement(
    Z = complete_ra(N),
    Y = Z*Y_Z_1 + (1-Z)*Y_Z_0) +
...
if(FALSE)
  design |> redesign(rho = seq(-1, 1, .4)) |>
  diagnose_design(sims = c(1, 2000, 1),
                  diagnosands = declare_diagnosands(
                    var_estimate = var(estimate),
                    mean_se_2 = mean(std.error^2))) |>
  write_rds(here::here("slides", "saved/conservative.rds"))
...
2.  the estimator is more conservative for negative correlations between $Y(0)$ and $Y(1)$ --- e.g. if those cases that do particularly badly in control are the ones that do particularly well in treatment, and
...
```{r, echo = FALSE}
read_rds(here::here("slides", "saved/conservative.rds")) |> tidy() |> 
  ggplot(aes(rho, estimate, color = diagnosand)) + 
...
ri2_out <- conduct_ri(
  ...
  p = "upper",
  sims = 4000
) |> write_rds(here::here("slides", "saved/ri2eg.rds"))
...
```{r, echo = FALSE}
read_rds(here::here("slides", "saved/ri2eg.rds")) |> summary() |> kable()
```
...
if(run)
 candidates |> sapply(get_p) |>
  write_rds(here::here("slides", "saved/ri_ci.rds"))

ci_df <- 
  data.frame(candidates = candidates, p = read_rds(here::here("slides", "saved/ri_ci.rds")))  
...
### `ri` with `DeclareDesign` (advanced) {.smaller}
...
if(run){
  ...
  design |> redesign(b = c(0, .25, .5, .75, 1)) |> 
  simulate_design(sims = c(200, 1, 1000, 1)) |>
  write_rds(here::here("slides", "saved/mde.rds"))
}

simulations <- read_rds(here::here("slides", "saved/mde.rds"))
```
...
if(run)
  ...
  write_rds(here::here("slides", "saved/ri_dd_inter.rds"))

read_rds(here::here("slides", "saved/ri_dd_inter.rds")) |> reshape_diagnosis() |> kable()
