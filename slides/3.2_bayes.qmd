...
```{r, out.width="70%", fig.width=12, fig.height=6.5, fig.align='center'}
...
```{r, warning = FALSE, echo = TRUE, comment = NA, echo = FALSE}
if(run)
...
With the full posterior you can can look at marginal posterior distributions over arbitrary transformations of parameters.

```{r, echo = TRUE, fig.asp=1, comment = NA}
summary((my_posterior$a + my_posterior$b)/my_posterior$a) |> tidy() |>
  kable(digits = 2)
```
...
Say that there is possible a village specific average outcome: $Y_v = a_v + b_vX$ where $a_v$ and $b_v$ are each draw from some distribution with a mean and variance of interest. The individual outcomes are draws from a village level  distribution centered on the village specific average outcome.
...
* There will be cooperation by *both* players  with probability 
...
A player with rationality $r_i$ will cooperate strategically with probability $r_i$ if $r_i<.5$ and 0 otherwise. 
Thus we are interested in $\int_0^{.5}r_i/\theta dr_i = .125/\theta$
