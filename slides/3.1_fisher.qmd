...
# Frequentist Analysis \label{L_inference} 
\hyperlink{ideas}{\beamergotobutton{Top}}

## Basic Analysis \label{nools}
...
Unbiased estimates of the (sample) average treatment effect can be estimated (\textbf{whether or not there imbalance on covariates}) using:
...
* The intuition for it comes straight from **sampling weights** --- you weight up in order to recover an unbiased estiamte of the potential outcomes for all units, whether or not they are assigned to treatment.
...
### Illustration: Estimating treatment effects with terrible treatment assignments: Fixer\label{Fixer}
...
n  <- 200; reps <- 500; X  <- runif(n)     # Create a covariate (length n)
Y  <- Y1 <- Y0 <- X                        # Say X completely determines Y!
Z  <- function(i) rank(X+2*runif(n))>(n/2) # Bad randomization! 
P  <- sapply(1:reps, Z)                    # Lots of possible draws
p  <- apply(P, 1, mean)                    # Recreate propensities!
pw <- (!P)*(1/(1-p)); pw[P]=(P*(1/p))[P]   # Create inv prop weights

naive   <- sapply(1:ncol(P),function(i) {
              mean(Y[P[,i]])-mean(Y[!P[,i]])}) 
weightd <- sapply(1:ncol(P),function(i) {  # IPW estimates
              weighted.mean(Y[P[,i]],  pw[,i][P[,i]])-
              weighted.mean(Y[!P[,i]], pw[,i][!P[,i]])}) 
```
...
```{r, echo = FALSE} 
par(mfrow=c(2,2));  
   plot(X, p, main="Propensities correlated with some covariate"); 
   plot(X[P[,1]], pw[,1][P[,1]],  
   main="Inverse propensity weights (Red=Control)"); 
   points(X[!P[,1]], pw[,1][!P[,1]], col="red")
   hist(naive, main="Distribution of possible estimates from naive analysis"); 
   hist(weightd, main="Distribution of estimates from weighted analysis")
```
...
### Demeaning interactions: Solution
...
As argued by Freedman (summary from @lin2012agnostic), we can get:  "worsened asymptotic precision, invalid measures of precision, and small-sample bias"^[though note that the precision concern does not hold when treatment and control groups are equally sized]
...
knitr::include_graphics("figs/cb.png")
```
...
```{r}
# df with rue treatment effect of 1 
...
### Randomization Inference
\small If you think hard about assignment you might come up with an allocation like this. 
...
\footnotesize  That allocation balances as much as possible. Given the allocation you might randomly assign individuals to different days as well as randomly assigning them to treatments within days. If you then figure out assignment propensities, this is what you would get:
...
\footnotesize Even under the assumption that the day of measurement does not matter, these assignment probabilities have big implications for analysis. 
...
* **Implications for analysis**: need to take account of propensities
 ...
 \textbf{Idea}: Wacky assignments happen but if you know the propensities you can do the analysis.
...
\scriptsize You can also estimate variance straight from the data. From \href{http://www.stat.berkeley.edu/~census/neyregcm.pdf}{Freedman Prop 1} (using combinatorics!) we have:
...
An illustration of \textit{how} conservative the conservative estimator of variance really is (numbers in plot are correlations between $Y(1)$ and $Y(0)$. 
...
* But as \href{http://arxiv.org/pdf/1405.6555.pdf}{Aronow, Green, and Lee (2014)}  point out, you \textit{do} know something. 
