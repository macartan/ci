...
* The key idea is that we can use the same procedure to assess the probability of the data igven a sharp null of no effect, but also a sharp null of *any other* **constant* effect. 
...
```{r, echo = FALSE}
ci_df |>
  ggplot(aes(candidates, p)) + geom_line() + theme_bw() +
  geom_hline(aes(yintercept = .05), color = "orange")
```

Warning: calculating confidence intervals this way is quite computationally intensive


### `ri` with `DeclareDesign`

`DeclareDesign` can do randomization inference natively. The trick is to ensure that the *only* stochastic component is the assignment to treatment when calculating the $p$ values (though if you are very careful you can also think through other sources of variability).
...
* Coming: impute all potential outcomes and use reveal

### RI in practice

In practice (unless you have a design declaration) it is a good idea to create a $P$ matrix when you do your randomization. This records the set of possible randomizations you might have had: or a sample of these.
