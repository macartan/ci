...
### Broader classes of estimands

The CATEs are conditional average treatment effects, for example the effect for men or for women.

However we might also imagine conditioning on unobservable or counterfactual features.

* The LATE (or CATE: complier average treatment effect) asks about the effect of a treatment ($X$) on an outcome ($Y$) *for people that are responsive to an encouragement* ($Z$)

$$LATE =  \frac{1}{|C|}\sum_{j\in C}(Y_j(X=1) - Y_j(X=0))\\ C:=\{j:X_j(Z=1) > X_j(Z=0) \}$$
* A *quantile* treatment effect: You might be interested in the difference between the median $Y(1)$ and the median $Y(0)$  (@imbens2015causal 20.3.1) or even be interested in the median $Y(1) - Y(0)$. Similarly for other quantiles.
 
### Broader classes of estimands

Many inquiries are averages of individual effects, even if the groups are not known, but they do not have to be:

* The RDD estimand is a statement about what effects *would be* at a threshold; it can be defined under a model even if no actual individuals are at the threshold. We imagine average potential outcomes as a function of treatment $Z$ and running variable $X$, $f(z, x)$ and  define: $$\tau_{RDD} := f(1, x*) - f(1, x*)$$
...
If it is not identifiable, then, even in the best case, we are not guaranteed getting the right answer.
...
If we can identify the controlled disteribution we can calculate other causal quantities of interest.
...
```{r, echo = FALSE}
m2 <- make_model("M1 -> Y <- M2; X <- Z -> M1")
plot(m2, x_coord = c(M2 = 3, Z=1, M1=3, X=2, Y=4), y_coord = c(M2=1, Z=1.5, M1=2, X=1, Y=1), nodecol = c(1,2,1,1,1))
```
...
* But we do not observe $T$, rather we observe $Z$. OK, but we can write: $$p(y|\hat{x}) = \sum_{t\in T} p(t) \sum_z p(y|\hat{x}, t, z)p(z|\hat{x}, pa(X))$$
...
This gives:  $$p(y|\hat{x}) = \sum_{pa(X)} p(pa(X)) \sum_z p(y|\hat{x}, z)p(z|pa(X)) $$
...
1. no element of $Z$ is a descendant (in the mutilated graph^[remove arrows pointing into $X$]) of any variable $W\not\in X$  which lies on a proper causal path from $X$ to $Y$^[A *proper* causal pathway nodes in $X$ to nodes in $Y$ only intersects $X$ at the endpoint] 
