...
lin_replication <-
  declare_model(N = 1000, z = runif(N, 0, 1),
...
```{r, eval = FALSE}
lin_replication_diagnosis <- lin_replication |> 
  redesign(nA = c(750, 600, 400, 250)) |>
  diagnose_design()
```

```{r, echo = FALSE}
if(params$run)
lin_replication |> redesign(nA = c(750, 600, 400, 250)) |>
  diagnose_design(sims = 10000) |> write_rds("saved/5.3.rds")

read_rds("saved/5.3.rds") |>
  tidy() |> filter(diagnosand %in% c("bias", "sd_estimate")) |>
  ggplot(aes(as.numeric(nA), estimate*1000, color = estimator))+
