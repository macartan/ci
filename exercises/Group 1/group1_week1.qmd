...
M1 <- 
  declare_model(
    ...
    X = rnorm(N),
    Y = rnorm(N),
    K = X + Y + rbinom(N, 1, prob = 0.5)) +
  declare_inquiry(ATE = mean(Y_X_1[K == 1] - Y_X_0[K == 1])) +
...
estimates <- conduct(M1, sims = 1000)
...
M1 <- 
  declare_model(
    ...
    X = rnorm(N),
    Y = rnorm(N),
    K = rbinom(N, 1, prob = plogis(0.5 + 0.5 * X))
  ) +
  declare_assignment(assign = complete_ra(N = 1000)) +
  declare_measurement(Y = reveal_outcomes(Y), K = reveal_outcomes(K)) +
  declare_estimator(
    formula = Y ~ X,
    model = lm_robust,
    subset = K == 1,
    label = "ATE_conditioned_on_K_1"
  ) +
  declare_inquiry(ATE = mean(Y[X > 0 & K == 1]) - mean(Y[X <= 0 & K == 1])) +
  declare_sampling(S = complete_rs(N = 1000, n = 500))

sim_data <- simulate_design(M1)
