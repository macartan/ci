...
```{r eval=FALSE}
results <- data.frame(N = numeric(), Posterior_Mean = numeric())

set.seed(1234)
for (N in seq(500, 6500, by = 500)) {
  data <- fabricate(N = N, 
                    X = rep(0:1, N/2), 
                    M = rbinom(N, 1, 0.05 + .9*X), 
                    Y = rbinom(N, 1, 0.05 + .9*M))

  updated_model <- model %>% update_model(data, refresh = 0)
  query_result <- query_model(
      model = updated_model, 
      using = c("priors", "posteriors"),
      query = "Y[X=1] - Y[X=0]"
  )
  posterior_mean <- query_result %>% 
    filter(using == "posteriors") %>% 
    pull(mean)

  results <- rbind(results, data.frame(N, Posterior_Mean = posterior_mean))
}
```

```{r eval=FALSE}
ggplot(results, aes(x = N, y = Posterior_Mean)) +
  geom_point() +
  theme_minimal()
```
